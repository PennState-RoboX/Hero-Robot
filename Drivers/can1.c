...
void CAN1_RX0_IRQHandler(void)
{
	...
	if(CAN_GetITStatus(CAN1,CAN_IT_FMP0)!= RESET)
	{
		...
		switch(RxMessage.StdId)
		{
			...
			case 0x20B: //云台电机yaw
			{
				motor_data[7].NowAngle = (RxMessage.Data[0]<<8)+RxMessage.Data[1];
				motor_data[7].Intensity = (RxMessage.Data[2]<<8)+RxMessage.Data[3];
				break;
			}
			case 0x208://拨弹电机
			{
...
{	
  ...
  TxMessage.DLC = 6;			//指定数据的长度
	TxMessage.Data[0] = -control_206>>8;
	TxMessage.Data[1] = -control_206;
	TxMessage.Data[2] = control_206>>8;
	TxMessage.Data[3] = control_206;
	TxMessage.Data[6] = control_2006>>8;
	TxMessage.Data[7] = control_2006;
  ...
  TxMessage.RTR = CAN_RTR_DATA;		//指定的帧将被传输的消息的类型   数据帧或远程帧
  TxMessage.DLC = 6;			//指定数据的长度
	TxMessage.Data[4] = control_205>>8;
	TxMessage.Data[5] = control_205;
